...
template <class T> inline constexpr T pow8(const T base) {
...
public:
  Mat(T *d, const int h, const int w)
      : data(d), width(w), height(h), is_data_allocated(false) {}
  Mat() : width(0), height(0), is_data_allocated(false) {}
  Mat(const int h, const int w) : width(w), height(h), is_data_allocated(true) {
    data = new T[w * h];
  }
  ~Mat() {
...
public:
  ...
  find_boxes(std::vector<std::array<int, 2>> &tour, int max_frames = 400) {
  ...
  std::vector<std::array<int, 2>> multiple_tours(int stripe_len = 250,
    ...
    for (unsigned int i = 0, i_e = single_tour.size(); i < i_e; ++i) {
      ...
      if (single_tour[i][0] > m / 2)
        x_off = -50;
      if (single_tour[i][1] > n / 2)
        y_off = -50;
      c2 = pow8(cost(single_tour[i][0] + x_off, single_tour[i][1] + y_off));

      if (c1 / c2 < 0.89)
        result.push_back(single_tour[i]);
    }
    // std::cout << "alive05" << std::endl;
    if ((unsigned int)n / 4 > result.size())
      result.clear();
  ...
  tour(const int off = 0, const int col_end = 0, const int overlap_info = 0,
    ...
    for (int r = 0; r < m; ++r)
      accumulate_costs(r, 0) = pow8(cost(r, off));
    if (ssp) {
